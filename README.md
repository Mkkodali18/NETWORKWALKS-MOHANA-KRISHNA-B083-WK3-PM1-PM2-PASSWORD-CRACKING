...
Run a dictionary (wordlist) attack against the hash.
Recover the plaintext password and use it to unlock the PDF.
Validate the same recovered passwords using John the Ripper's GUI front-end, Johnny.
Capture the lab completion flags.
...
```
![Flag 2 — networkwalks persistence flag](images/06-flag-networkwalks-persistence.png)
```
nw{networkwalks_persistence_jtr_270521}
```
---
📚 Key Takeaways
Dictionary attacks are fast against weak passwords. Both target passwords fell within the first ~100 attempts of a small wordlist.
`pdf2john` (and equivalents) turn document encryption into a crackable hash format that standard password-cracking tools understand.
Patience + the right wordlist is genuinely most of the battle — as the lab's own tagline puts it, cracking passwords is about persistence and using a well-targeted dictionary.
Strong passwords (long, random, non-dictionary, non-pattern) are what actually defeat this class of attack — a lesson best learned by seeing how easily weak ones break.
---
🙏 Credits
Lab designed and provided by Networkwalks as part of their Cyber Security training track.
Cracking performed with John the Ripper / Johnny GUI.
Hash extraction via an online `pdf2john`-based conversion tool.
